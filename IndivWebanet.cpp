...
void GenerationHandler( vector<vector<int>> current ) // Вывод текущего поколения на экран, запись текущего поколения в файл "work.out"
{
    ...
    for ( vector<int> row : current )
    {
        for ( int elem : row )
        {
            if ( elem == 1 ) // Если в ячейке есть живой микроб уровня 1.
            {
                outputFile << 'X'; // Запись 'X' для живых микробов.
                cout << 'X'; // Вывод 'X' для живых микробов.
            }
            else
            {
                outputFile << ' '; // Запись пробела для мертвых микробов.
                cout << ' '; // Вывод пробела для мертвых микробов.
            }
        }
...
int main()
{
    ...
    for ( int generation = 1; generation <= numGenerations; generation++ ) // Пеоебор поколений.
    {
        ...
        GenerationHandler( current ); // Вывод текущего поколения на экран, запись текущего поколения в файл.
